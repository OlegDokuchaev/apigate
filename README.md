...
3. ApiGate optionally extracts typed query parameters.
4. `before` hooks run in order.
5. The route optionally validates or maps `json` or `form` body data.
6. A routing strategy selects candidate backends.
7. A balancer picks one backend.
8. ApiGate rewrites the URI and proxies the request to the upstream.
9. Runtime events are emitted only if a runtime observer is configured.
...
- `query` validation does not read the body and stores typed query data in `RequestScope`.
...
| `hooks` | Shared state, auth, header/query mutation, hook chains, per-request scope data. |
...
| `map` | JSON and form transformations, validate-only maps (`-> ()`), raw-byte signature checks beside a typed input, and schema-less `RawBody` maps. |
